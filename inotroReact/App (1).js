...
class App extends React.Component {
  state = {
    ...
    // Abbiamo due modi per creare la lista degli utenti
    // 1- Creazione dei dati tramite oggetti js (json)
    // users:[
    ...
    // 2- Creazione dei dati tramite oggetti js, creati attraverso le classi
  ...
  render() {
    if(this.state.page == 0) {
      return <div>
        <h1>Homepage</h1>
        <ul>{this.state.users.map(user => this.userRow(user))}</ul>
      </div>
    }
